...
public class AdvancedPhysixMove : MonoBehaviour {
	...
	*/
	...
	void Update () {
		...
		//Jump and WallJump
		bool moving = Input.GetKey("a") || Input.GetKey("d") || Input.GetKey("w") || Input.GetKey("s"); //should we be moving?
		if(physix.IsColliding("DownHit") && Input.GetKeyDown("space")){physix.ApplyMovement("Global", JumpSpeed, AxisType.y, ValueType.value); VisualCharacter.localScale = new Vector3(0.5f,1.5f,0.5f);}
		...
		//Physics Stuff
		if(fixedupdate){
			if(physix.IsColliding("DownHit")){physix.ApplyMovement("Normal", 0f, AxisType.y, ValueType.value);} //Simple way to keep character on the ground.
			...
			if(moving){
				Vector3 look = (Input.GetKey("a") ? -Vector3.right : Vector3.zero) +
						   		(Input.GetKey("d") ? Vector3.right : Vector3.zero) +
						   		(Input.GetKey("w") ? Vector3.forward : Vector3.zero) +
						   		(Input.GetKey("s") ? Vector3.back : Vector3.zero);
