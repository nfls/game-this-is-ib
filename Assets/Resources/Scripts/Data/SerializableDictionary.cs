...
public class SerializableDictionary<TKey, TValue> : IDictionary<TKey, TValue> {
    ...
    public SerializableDictionary(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey> comparer) : this(dictionary?.Count ?? 0, comparer) {
        if (dictionary == null)
            throw new ArgumentNullException("dictionary");

        foreach (KeyValuePair<TKey, TValue> current in dictionary) Add(current.Key, current.Value);
    }

    public bool ContainsValue(TValue value) {
        if (value == null) {
            for (int i = 0; i < _Count; i++)
                if (_HashCodes[i] >= 0 && _Values[i] == null) return true;
        } else {
            var defaultComparer = EqualityComparer<TValue>.Default;
            for (int i = 0; i < _Count; i++)
                if (_HashCodes[i] >= 0 && defaultComparer.Equals(_Values[i], value)) return true;
        }
    ...
    private void Resize(int newSize, bool forceNewHashCodes) {
        ...
        for (int i = 0; i < bucketsCopy.Length; i++) bucketsCopy[i] = -1;
        ...
        if (forceNewHashCodes)
            for (int i = 0; i < _Count; i++)
                if (hashCodesCopy[i] != -1) hashCodesCopy[i] = _Comparer.GetHashCode(keysCopy[i]) & 2147483647;
    ...
    private void Resize() => Resize(PrimeHelper.ExpandPrime(_Count), false);
    ...
    public void CopyTo(KeyValuePair<TKey, TValue>[] array, int index) {
        ...
        for (int i = 0; i < _Count; i++)
            if (_HashCodes[i] >= 0) array[index++] = new KeyValuePair<TKey, TValue>(_Keys[i], _Values[i]);
    }
...
[Serializable]
public class StringDIntDictionary : SerializableDictionary<string, DInt> { }

[Serializable]
public class StringDLongDictionary : SerializableDictionary<string, DLong> { }

[Serializable]
public class StringDFloatDictionary : SerializableDictionary<string, DFloat> { }